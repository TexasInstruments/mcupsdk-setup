...
patch_ccs_j722s() {
    echo "Patching CCS for J7AEN..."
    local patch_file=J7AEN_CSP.zip
    local ccs_version=$1
    local install_dir=$2
    local ccs_folder=ccs`echo ${ccs_version} | cut -d "." -f -3 | sed -e "s|\.||g"`

    wget  -v http://bangsdowebsvr01.india.ti.com/PROCESSOR_SDK_RTOS_AUTOMOTIVE/swdownloads/${patch_file}
    unzip ${patch_file}
    cp -rT ccs/ccs_base ${install_dir}/${ccs_folder}/ccs/ccs_base
    rm -rf ./ccs
}
...
c7x_url_folder="${CGT_TI_C7X_VERSION}.${CGT_TI_C7X_VERSION_SUFFIX}"
c7x_install_folder="ti-cgt-c7000_${CGT_TI_C7X_VERSION}.${CGT_TI_C7X_VERSION_SUFFIX}"
c7x_install_file="ti_cgt_c7000_${CGT_TI_C7X_VERSION}.${CGT_TI_C7X_VERSION_SUFFIX}_linux-x64_installer.bin"
